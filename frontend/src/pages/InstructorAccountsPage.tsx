...
const DEFAULT_INSTRUCTOR_TABS = ['schedule', 'attendance', 'grades', 'profile'];
...
function printReceipt(name: string, username: string, password: string) {
  ...
  if (!w) return;
  w.document.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>حساب محاضر</title><style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Cairo', 'Segoe UI', sans-serif; background: #fff; padding: 40px 30px; color: #222; }
    .logo { text-align: center; margin-bottom: 20px; }
    .logo h2 { font-size: 1.6rem; color: #6366f1; }
    .logo p { font-size: 0.85rem; color: #666; }
    h1 { text-align: center; font-size: 1.2rem; margin: 16px 0; color: #333; border-bottom: 2px solid #6366f1; padding-bottom: 12px; }
    .card { border: 2px dashed #6366f1; border-radius: 16px; padding: 28px 24px; margin: 24px 0; background: #f5f3ff; }
    ...
    <div class="logo"><h2>📚 بيانات حساب المحاضر</h2><p>بوابة المحاضر الإلكترونية</p></div>
    <h1>${name}</h1>
    <div class="card">
...
export const InstructorAccountsPage = () => {
  ...
  const [selectedTabs, setSelectedTabs] = useState<string[]>(DEFAULT_INSTRUCTOR_TABS);
  ...
  useEffect(() => {
    ...
    if (filterType === 'with_account') {
      const ids = new Set(users.filter(u => u.instructorId).map(u => u.instructorId));
    ...
    setFiltered(list);
  }, [instructors, users, query, filterType]);

  const existingUser = (iid: string) => users.find(u => u.instructorId === iid);

  const openCreate = (inst: any) => {
    setSelectedInstructor(inst);
    const eu = existingUser(inst.id);
    setSelectedTabs(eu?.portalTabs?.length ? eu.portalTabs : DEFAULT_INSTRUCTOR_TABS);
    setPassword(eu ? '' : genPassword());
    setAutoUsername(eu ? eu.username : generateUsername());
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    const eu = existingUser(selectedInstructor.id);
    ...
    try {
      if (eu) {
        ...
        if (password) {
          printReceipt(selectedInstructor.name, selectedInstructor.id, password);
        }
      } else {
        const result = await apiFetch('/auth/users', {
          ...
          body: JSON.stringify({
            ...
            password,
            fullName: selectedInstructor.name,
            role: 'INSTRUCTOR',
            status: 'ACTIVE',
            portals: ['INSTRUCTOR'],
            portalTabs: selectedTabs,
        ...
        toast.success('تم الإنشاء', `حساب المحاضر ${selectedInstructor.name}`);
        printReceipt(selectedInstructor.name, autoUsername, password);
        setUsers(prev => [...prev, {
          id: result.id,
          username: autoUsername,
          fullName: selectedInstructor.name,
  ...
  const filteredStats = {
    ...
    withAccount: users.filter(u => u.instructorId).length,
    withoutAccount: instructors.length - users.filter(u => u.instructorId).length,
  };

  const eu = selectedInstructor ? existingUser(selectedInstructor.id) : null;

  const generateUsername = (): string => {
    const existing = users.filter(u => u.instructorId && u.username?.startsWith('T'));
    let maxNum = 5200000;
    for (const u of existing) {
      const num = parseInt(u.username.replace('T', ''), 10);
  ...
  return (
    <PermissionGuard perm="admin.instructors.view">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: '1 1 180px', minWidth: 150 }}>
              ...
              <input type="text" className="glass-input" style={{ paddingRight: 36, fontSize: '0.85rem' }}
                placeholder="اسم / رقم النظام / تخصص..." value={query}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 10 }}>
          {[
            { label: 'إجمالي', value: filteredStats.total, color: 'blue' as const },
            { label: 'لديه حساب', value: filteredStats.withAccount, color: 'green' as const },
            { label: 'بدون', value: filteredStats.withoutAccount, color: 'purple' as const },
          ].map(c => (
            ...
            <table className="glass-table">
              ...
              <tbody>
                {filtered.map((inst, idx) => {
                  const eu2 = existingUser(inst.id);
                  return (
                    <tr key={inst.id}>
                      <td style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{idx + 1}</td>
                      <td style={{ fontFamily: 'monospace', fontSize: '0.82rem', direction: 'ltr' }}>{inst.id}</td>
                      <td style={{ fontWeight: 600, fontSize: '0.88rem' }}>
                        {inst.name}
                        {inst.specialization && <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 400 }}>{inst.specialization}</div>}
                      </td>
                      <td style={{ fontSize: '0.8rem', color: 'var(--primary-color)' }}>{inst.specialization || '—'}</td>
                      <td style={{ fontSize: '0.8rem' }}>{inst.phone || '—'}</td>
                      <td>
                        {eu2
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 4 }}>
                          {hasPermission('admin.instructors.add') && (
                            <button className="glass-btn secondary sm" onClick={() => openCreate(inst)} style={{ padding: '5px 10px' }}
                              title={eu2 ? 'تعديل' : 'إنشاء حساب'}>
                              {eu2 ? <Shield size={13} /> : <UserPlus size={13} />}
                            </button>
                          )}
                          {eu2 && hasPermission('admin.instructors.delete') && (
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32, opacity: 0.5 }}>لا يوجد محاضرون</td></tr>
                )}
              </tbody>
            </table>
          </div>
            ...
            <div className="glass-panel" style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
                <h3 style={{ margin: 0, fontSize: '1rem' }}>
                  {eu ? 'إعدادات حساب المحاضر' : 'إنشاء حساب محاضر'}
                </h3>
              ...
              <div style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>{selectedInstructor.name}</div>
              ...
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontWeight: 600, marginBottom: 6, fontSize: '0.85rem' }}>التبويبات المرئية للمحاضر</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: 8 }}>
                  اختر ما يمكن للمحاضر مشاهدته
                </div>
        ...
        <ConfirmModal
          ...
          message="حذف حساب المحاضر؟"
          subMessage="سيتم حذف حساب المستخدم المرتبط بالمحاضر"
          confirmText="حذف"
          danger
